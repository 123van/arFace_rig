...
def faceSkinUI():
    ...
    #create window
    window = cmds.window( 'faceSkinUI', title = 'faceSkin UI', w =420, h =500, mnb = True, mxb = True, sizeable=True, nestedDockingEnabled = True, resizeToFitChildren = True )

    tabs = cmds.tabLayout(innerMarginWidth=5, innerMarginHeight=5)
    
    #rowColumnLayout
    child1= cmds.rowColumnLayout( numberOfColumns = 4, bgc=[.22,.3,.40], columnWidth = [(1, 120),(2, 120),(3, 120), (4, 40)], columnOffset = [(1, 'right', 5)] )
    ...
    cmds.text( label = 'move hierachy to loc' )
    ...
    cmds.button( label = 'update_faceMain', bgc=[.42,.5,.60], command = update_faceMain )
    cmds.button( label = 'update_clsFaceMain', bgc=[.42,.5,.60], command = update_clsFaceMain )
    cmds.text( label = '')
    
    cmds.button( label = '?', command = updateCtlPlace_helpImage )
    cmds.button( label = 'update_ctl_Placement', bgc=[.42,.5,.60], command = update_CtlsPlacement, ann = "shape CTLCrv to hiCrv and run" ) 
    cmds.text( label = ' :shape CTLcrv to Hi_crv_' )
    cmds.text( label = 'new/run')
    ...
    cmds.floatField( "ctl_color", value = 10 )
    cmds.text( label = '')
    ...
    cmds.button( label = ' twitch update weight ', bgc=[.5,.6,.4], command = twitchUpdateWeight )
    cmds.text( label = '')
    ...
    cmds.text( label = '')
    cmds.text( label = '       create joint chain')
    cmds.text( label = ' for tongue                    ')
    cmds.text( label = '')
    cmds.text( label = ' skin using delta mush')
    ...
    cmds.text( label = '')
    cmds.text( label = 'Squash & Stretch', bgc = [.12,.2,.30], fn = "boldLabelFont",height= 20 )
    cmds.text( label = '')
    cmds.text( label = '')
    spaceBetween(1,4)
    ...
    cmds.text( label = '')
    cmds.button( label = 'lattice Squach skinning', bgc=[.42,.5,.60], command = latticeSquachskinning )
    ...
    cmds.text( label = '')    
    cmds.text( label = '      optional      ', bgc = [.12,.2,.30], height= 20 )    
    cmds.text( label = '')
    cmds.text( label = '')
    
    #simple scale squach
    objName = cmds.textField( 'indieSquach', text = "mouth" )
    #x+, xdown, yup, ydown, zup, zdown, none.
    cmds.optionMenu('Axis', changeCommand= printNewMenuItem )
    cmds.menuItem( label= "x" )
    cmds.menuItem( label= "y" )
    cmds.menuItem( label= "z" )
    cmds.button( label = 'indieScaleSquach', bgc=[.42,.5,.60], command = indieScaleSquach )
    cmds.text( label = '')
    
    #cmds.text( label = '     create "mouthSquach',  bgc = [.10,.4,.40], height= 20 )
    #cmds.text( label = 'obj&"mouthSquach_ctl"',  bgc = [.10,.4,.40], height= 20 )
    #cmds.text( label = '')
 
    cmds.text( label = '     select obj',  bgc = [.10,.4,.40], height= 20 )
    cmds.text( label = 'and "mouthSquach_ctl"    ',  bgc = [.10,.4,.40], height= 20 )
    cmds.button( label = 'mouthSquach_setup', bgc=[.42,.5,.60], command = mouthSquach_setup )
    cmds.text( label = '')
    spaceBetween(1,4)
    ...
    cmds.text( label = ' jointChain_title : ' )
    cmds.text( label = 'jointChain_length :' )
    cmds.text( label = '')
    cmds.text( label = '')
    
    wireName = cmds.textField( 'jointChain_title', text = "hair01" )    
    jntLen = cmds.intField( 'jointChain_length', value = 6 )
    cmds.button( label = 'createJntAlongCrv', bgc=[.42,.5,.60], command = createJntAlongCrv )
    cmds.text( label = '')
    cmds.text( label = '' )    
    cmds.text( label = '         select base jnt /', fn = "boldLabelFont" )
    cmds.text( label = ' end jnt for splineIk         ', fn = "boldLabelFont" )
    cmds.text( label = '')
    
    cmds.text( label = "number of hairClts", fn = "boldLabelFont"  )
    cmds.optionMenu('hairClts_Number', changeCommand= printNewMenuItem )
    cmds.menuItem( label= "2" )
    cmds.menuItem( label= "3" )
    cmds.menuItem( label= "4" )
    cmds.menuItem( label= "5" )
    cmds.menuItem( label= "6" )
    cmds.button( label = 'dyn-hair Setup', bgc=[.42,.5,.60], command = dyn_hairSetup )
    cmds.button( label = '?', command = hairSetup_helpImage )
    ...
    cmds.text( label = '')
    cmds.button( label = 'miscellaneous', bgc=[.42,.5,.60], command = miscellaneousUI )
    spaceBetween(1,2)
...
def squachSetup_helpImage(*pArgs):
    arHelpImage( "squachSetup02" )

def nonTwitchSquach_helpImage(*pArgs):
    arHelpImage( "nonTwitchSquach03" )

def hairSetup_helpImage(*pArgs):
    arHelpImage( "hair_setup02" )
...
def update_faceMain( *pArgs ):
    faceCompFunction.update_faceMain()

def update_clsFaceMain( *pArgs ):
    faceCompFunction.update_clsFaceMain()
...
def attachCurves(*pArgs):
    ...
    crvSel = cmds.ls(sl=1)
    if len(crvSel)==2:
        faceCompFunction.attachCrvs(crvSel)
    else:
        cmds.confirmDialog( title='Confirm', message="select 2 curves" )
...
def miscellaneousUI(*pArgs):
    from twitchScript import miscellaneous
...
def latticeSquachskinning(*pArgs):
    jntLenNum = cmds.intField( 'joint_length', q=1, value = 1 )    
    squachSetup.latticeSquachskinning(jntLenNum)
...
def indieScaleSquach(*pArgs):
    squachNode = cmds.ls(sl=1)[0]
    name = cmds.textField( 'indieSquach', query=True, text=True )
    axis = cmds.optionMenu( 'Axis', q=1, value = 1 )
    squachSetup.indieScaleSquach(squachNode, axis, name )
    
    
def mouthSquach_setup(*pArgs):
    mySel = cmds.ls(sl=1, typ = "transform")
    head = mySel[0]
    ctl = mySel[1] 
    squachSetup.mouthSquach_setup( head, ctl )
...
def createJntAlongCrv(*pArgs):    
    title = cmds.textField( 'jointChain_title', query=True, text=True )
    numJnt = cmds.intField( 'jointChain_length', q=1, value = 1 )
    faceCompFunction.createJntAlongCrv( numJnt, title )

def dyn_hairSetup( *pArgs ):
    jnts= cmds.ls(sl=1)
    title = jnts[0].split('_')[0]
    jntList = cmds.getAttr('helpPanel_grp.' + title + '_jntList' )
    print jntList
    ctlLen = cmds.optionMenu( 'hairClts_Number', q=1, value = 1 )
    print ctlLen
    faceCompFunction.dyn_hairSetup( jntList, ctlLen )
...
def twitchUpdateWeight( *pArgs ):
    splitRange = cmds.optionMenu('split_range', query=True, value=True)
    blendShapeMethods.twitchUpdateWeight( splitRange )
