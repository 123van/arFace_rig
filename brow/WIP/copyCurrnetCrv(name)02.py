...
def copyCurrnetCrv(name):
    ...
    cntMult = cmds.ls("c_browCrvMult*", type = "multiplyDivide")
    RotateScale = cmds.getAttr (cntMult[0] + ".input2X") 
    ...
    for x in range(0, browLen):
        ...
        posX = rotY/RotateScale + initialX
        posY = rotX/-RotateScale   
    ...
    jntCtl = cmds.listRelatives ( cmds.ls("*_brow*P", fl = 1, type = "transform" ), c=1, type = "transform" ) 
    ctlLen = len(arcCtls)
