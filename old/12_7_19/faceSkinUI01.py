...
def faceSkinUI():
    ...
    cmds.text( label = 'num of eyeCtls:')
    cmds.intField( "numOfCtls", bgc=[0,0,0], value = 5 )
    cmds.menuItem( label=3 )
    cmds.menuItem( label=5 )
    cmds.menuItem( label=7 )
    cmds.text( label = '')
    cmds.text( label = '')
    cmds.text( label = 'shape CTLCrv to HiCrv')
    cmds.text( label = '')
    cmds.text( label = '')
    ...
    cmds.text( label = '')
    cmds.text( label = 'src crv( bs with no targets)')
    cmds.text( label = 'and tgt crv')
    cmds.text( label = 'copy blendShape delta')
    cmds.button( label = 'copyCrvBS delta', bgc=[.42,.5,.60], command = copyCrvDeltaBS )
    cmds.button( label = 'bakeCrvBS delta', bgc=[.42,.5,.60], command = bakeCrvBSDelta )
    cmds.text( label = '')
    ...
    cmds.button( label = 'vertices ToCurve', command = shapeToCurve )
    cmds.separator( h = 15)    
    cmds.separator( h = 15)
    cmds.separator( h = 15)
    cmds.text( label = '')
    ...
    cmds.tabLayout( tabs, edit=True, tabLabel=((child1, 'Run It All'), (child2, 'Face Lift'), (child3, 'Face Skinninng')) )
    #show window
...
def eyeCtlCrv(*pArgs):
	numEyeCtl = cmds.intField( "numOfCtls", q=True, value = True ) 
	prefix = ["l_up", "l_lo", "r_up", "r_lo"] 
	for pre in prefix:
		faceCompFunction.eyeCtrlCrv( pre, numEyeCtl )
...
def jawJoints(*pArgs):
    for ud in ["up","lo"]:
        faceCompFunction.mouthJoint( ud )

def jawCrvConnect(*pArgs):
    for ud in ["up","lo"]:

        faceCompFunction.mouthCrvToJoint( ud ) #connect crv's poc to jaw_jnts  
        faceCompFunction.lipCtlSetup( ud ) # create "lipBS_grp" / "_lipBS_crv" blendShape

    faceCompFunction.indieCrvSetup() 
def jawCtl_setup():
    #lipBS / jawDrop / jawOpen Crv with "joints" setup ( check if expression on upMid_jawOpen )
    faceCompFunction.indieCrvSetup()
...
def copyCrvDeltaBS(*pArgs):
	faceSkinFunction.copyCrvDeltaBS()

def bakeCrvBSDelta(*pArgs):
	faceSkinFunction.bakeCrvBSDelta()
...
def eyeDirectionOffset( *pArgs ):
    faceCompFunction.eyeDirectionOffset()        
