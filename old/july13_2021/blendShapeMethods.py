...
def transferCurveBS( srcCrv, tgtCrv, name ):
    ...
    wireCrv = cmds.rename( tgtCrv, 'lipShape_wire' )
    srcCvs = cmds.ls( srcCrv + ".cv[*]", fl=1 )
    ...
    bakeCrvDeltaBS( srcCrv, wireCrv, name )
    targetGrp = bakeTarget_reconnect(wireCrv)
    ...
    if lipCrv_pos[0] < 0.001:
        ...
        pm.confirmDialog( title='Confirm', message=' %s positionX is not zero'%wireCrv )    
