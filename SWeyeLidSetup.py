...
def lidDetailCtl():
    ...
    for updn in LRUpLow:
        ...
        for i in range (1, lidJntsLen+1):
            ...
            cmds.setAttr ( ctlYPOC + '.parameter', incrementPoc *i )
            
            # POC on ctlCrv drive detail control parent  
            cntRemoveX = cmds.shadingNode ( 'addDoubleLinear', asUtility=True, n= updn +"Cnt"+ str(i)+"RemoveX" )
            momMult = cmds.shadingNode ( 'multiplyDivide', asUtility=True, n = updn +'Mom'+ str(i)+'_mult' )
            cmds.connectAttr ( ctlYPOC +".positionY", detailPs[i-1] + ".ty" )
            
            #Xvalue match between POC and CtrlP ( detailPs[i-1] = 2*ctlPoc -1 )
            cmds.connectAttr ( ctlYPOC +".positionX", momMult + ".input1X" )        
            cmds.connectAttr ( momMult + ".outputX", cntRemoveX + ".input1" )
            cmds.setAttr ( momMult + ".input2X", 2 )
            cmds.setAttr (  cntRemoveX  + ".input2", -1 ) 
            cmds.connectAttr ( cntRemoveX +".output", detailPs[i-1] + ".tx" )
            ...
            '''cornerYPOC = cmds.shadingNode ( 'pointOnCurveInfo', asUtility=True, n = updn + 'CornerYPoc' + str(i).zfill(2)) 
            cmds.connectAttr ( updn +"CornerCrv.worldSpace", cornerYPOC + '.inputCurve' )   
            cmds.setAttr ( cornerYPOC + '.turnOnPercentage', 1 )        
            cmds.setAttr ( cornerYPOC + '.parameter', incrementYPoc*(i))'''   
...
def eyeCtl_EXP():
    ...
    for LR in prefix:
        ...
        cmds.connectAttr ( maxCvs[-1] + '.xValue', outCornerSum + '.input2D[3].input2Dx' )
        cmds.setAttr( outCornerSum + '.input2D[4].input2Dx', -1 )
        ...
        cmds.connectAttr ( outCornerMult + '.outputY', LR + 'cnrWide02_jnt.ry' )        
        ...
        cmds.connectAttr ( maxCvs[0] + '.xValue', inCornerSum + '.input2D[3].input2Dx' )
        ...
        cmds.connectAttr ( inCornerMult + '.outputY', LR + 'cnrWide01_jnt.ry' )          
    ...
    for UD in UDLR:
        ...
        for i in range( 0, length ):        
            ...
            if '_lo' in UD:
                ...
                # add corner curve to wide jnt 
                twist_addWide = cmds.shadingNode ('addDoubleLinear', asUtility=True, n = UD + 'TwistWideAdd' + str(i+1).zfill(2))

                cmds.connectAttr( cornerPOC + '.positionY', twist_addWide + '.input1' )            
                cmds.connectAttr( loStartCon +'.outColorG', twist_addWide + '.input2') 
                cmds.connectAttr( twist_addWide + '.output', jntMult + '.input1X' )
                # -'lidFactor.lidRotateX_scale' (= rotX_invert + '.outputX')
...
def blinkRemap():
    ...
    for UD in UDLR:
        ...
        for i in range( 0, length ):
            ...
            # wideSum to wide jnt
            cmds.connectAttr( cornerPOC + '.positionY', twist_addWide + '.input1' )            
            cmds.connectAttr( upStartCon +'.outColorG', twist_addWide + '.input2') 
            cmds.connectAttr( twist_addWide + '.output', jntMult + '.input1X' )
            cmds.connectAttr( rotX_invert + '.outputX', jntMult + '.input2X')
            cmds.connectAttr( jntMult +'.outputX', wideJnts[i] + '.rx' )  
