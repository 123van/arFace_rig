        ...
        for f in range(2, len(rootBrowSkin)-2 ):
            ...
            if browJnt in childJnts:
                ...
                for dict in rootBrowSkin[f]:
                    ...
                    wgtValue= float(dict.attrib['value'])
                    #if vertIndex in browTZNum:
                    print browTZWgt[vertIndex] 
                    cmds.setAttr ( skinCls+'.wl['+ str(vertIndex) + "].w[" + str(childJntID)+ "]", ( min(1, browTZWgt[vertIndex]) * wgtValue))                
             
                    baseVal = max( ( browUpWgt[vertIndex]- browDnWgt[vertIndex] ), 0 )
                    cmds.setAttr ( skinCls+ '.wl['+ str(vertIndex) + "].w[" + str(baseJntID) + "]", baseVal*wgtValue )
                    
                    roYVal = max( ( browUpWgt[vertIndex] - browTZWgt[vertIndex] ), 0 )
                    cmds.setAttr ( skinCls + '.wl['+ str(vertIndex) + "].w[" + str(rotYJntID) + "]", roYVal*wgtValue )                    
...
#zero out 'browJnts weight' / transfer to headSkel_jnt
def zero_browWeight_headSkel():
    vertSel = cmds.ls(sl=1)
    headGeo = vertSel[0].split('.')[0]
    headSkin =mel.eval("findRelatedSkinCluster %s"%headGeo )

    browBase = cmds.ls("*_browBase*_jnt", type = "joint" )
    #browWide = cmds.ls("*_browWide*_jnt", type = "joint" )
    browDown = cmds.listRelatives( browBase, c=1, type = "joint" )
    browRotY = cmds.listRelatives( browDown, c=1, type = "joint" )
    headSkel = 'headSkel_jnt'
    browTipList = lastJntOfChain( browBase )
    browJnts = browBase + browRotY + browTipList
    # Use zip to loop through both old and new joints.
    for bj in browJnts:
        cmds.setAttr( bj + '.liw', 0 )
        cmds.skinPercent( headSkin, tmw=[bj, headSkel ]) # Transfer weights from the old joint to the new one.
        cmds.setAttr( bj + '.liw', 1 )
    # Clear vertex selection.
    cmds.select(clear=True)


def ArFace_copyBrowSkinWeight():
    headGeo = cmds.ls(sl=1)
    headSkin =mel.eval("findRelatedSkinCluster %s"%headGeo[0] )
    #get the xml file for skinWeight ( brow/ eyeLid / lip )
    dataPath = cmds.fileDialog2(fileMode=3, caption="set directory")

    if os.path.exists(dataPath[0] + '/headSkinWgt01.xml'):      
        headSkinWeight = ET.parse( dataPath[0] + '/headSkinWgt01.xml')
        rootHeadSkin = headSkinWeight.getroot()
        browBase = cmds.ls("*_browBase*_jnt", type = "joint" )
        browWide = cmds.ls("*_browWide*_jnt", type = "joint" )
        browDown = cmds.listRelatives( browBase, c=1, type = "joint" )
        ...
        browTipList = lastJntOfChain( browBase )
        cmds.setAttr( 'headSkel_jnt.liw', 0 )
        for c in range(2, len(rootHeadSkin)-2 ):
            headJnt = rootHeadSkin[c].attrib['source']	    
            if headJnt in browBase:
                baseJnt = headJnt
                baseJntID = getJointIndex( headSkin, baseJnt )
                for dict in rootHeadSkin[c]:
                    vertID = int(dict.attrib['index'])    		    
                    weight= float(dict.attrib['value'])
                    cmds.setAttr( baseJnt + '.liw', 0 )
                    cmds.skinPercent( headSkin, headGeo[0] +'.vtx[%s]'%vertID, nrm=1, transformValue=[baseJnt, weight] )            	
                    #cmds.setAttr ( headSkin + '.wl['+ str(vertID) + "].w[" + str(baseJntID)+ "]", weight )
                    cmds.setAttr( baseJnt + '.liw', 1 )
                    
            elif headJnt in browDown:
                downJnt = headJnt
                downJntID = getJointIndex( headSkin, downJnt )
                for dict in rootHeadSkin[c]:
                    vtxID = int(dict.attrib['index'])
                    wgt= float(dict.attrib['value'])
                    cmds.setAttr( downJnt + '.liw', 0 )
                    cmds.skinPercent( headSkin, headGeo[0] +'.vtx[%s]'%vtxID, nrm=1, transformValue=[downJnt, wgt] )
                    cmds.setAttr( downJnt + '.liw', 1 )
                                
            elif headJnt in browRotY:
                RotYJnt = headJnt
                for dict in rootHeadSkin[c]:
                    vxID = int(dict.attrib['index'])
                    vWgt= float(dict.attrib['value'])
                    cmds.setAttr( RotYJnt + '.liw', 0 )
                    cmds.skinPercent( headSkin, headGeo[0] +'.vtx[%s]'%vxID, nrm=1, transformValue=[RotYJnt, vWgt] )
                    cmds.setAttr( RotYJnt + '.liw', 1 )
                                    
            elif headJnt in browTipList:
                tipJnt = headJnt
                #tipJntID = getJointIndex( headSkin, tipJnt )
                for dict in rootHeadSkin[c]:
                    vertexID = int(dict.attrib['index'])
                    vtxWgt= float(dict.attrib['value'])
                    cmds.setAttr( tipJnt + '.liw', 0 )
                    cmds.skinPercent( headSkin, headGeo[0] +'.vtx[%s]'%vertexID, nrm=1, transformValue=[tipJnt, vtxWgt] )
                    cmds.setAttr( tipJnt + '.liw', 1 )
                      
            elif headJnt in browWide:
                wideJnt = headJnt
                #browWideID = getJointIndex( headSkin, wideJnt )
                for dict in rootHeadSkin[c]:
                    vertID = int(dict.attrib['index'])
                    vertWgt= float(dict.attrib['value'])
                    cmds.setAttr( wideJnt + '.liw', 0 )
                    cmds.skinPercent( headSkin, headGeo[0] +'.vtx[%s]'%vertID, nrm=1, transformValue=[wideJnt, vertWgt] )
                    cmds.setAttr( wideJnt + '.liw', 1 )
...
def ArFace_copyEyeLidSkinWeight():
    ...
    if os.path.exists(dataPath[0] + '/headSkinWgt01.xml'):      
        headSkinWeight = ET.parse( dataPath[0] + '/headSkinWgt01.xml')
        rootHeadSkin = headSkinWeight.getroot()
        lidBlink = cmds.ls("l_*LidBlink*_jnt", type = "joint" )
        lidWide = cmds.ls("l_*LidWide*_jnt", type = "joint" )
...
def tongueSetup():
    ...
    name = mySel.split("_")[0]
    baseJnt =  cmds.rename( mySel , name + "Base_jnt" )
    baseJPos = cmds.xform( baseJnt, q=1, ws=1, t=1  )
    null = cmds.group (em=1, p = baseJnt, n = baseJnt +"P")
    cmds.setAttr( null + ".t", 0,0,0)
    cmds.setAttr( null + ".r", 0,0,0)
    cmds.parent( null, w=1 )
    cmds.parent( baseJnt, null )
    
    jnts = cmds.listRelatives(baseJnt, ad=1, typ = "joint" )
    
    scndJnt = cmds.listRelatives(baseJnt, c=1, typ = "joint" )[0]
    scndPos = cmds.xform( scndJnt, q=1, ws=1, t=1  )
    radius = distance( baseJPos, scndPos )
    #base controller
    baseCtl= genericController( name + "Base_ctl", baseJPos, radius*0.5, "circle", 11 )
    cmds.parentConstraint( baseCtl, baseJnt, maintainOffset = 1 )
    ctls = []
    for i in range(len(jnts)):
        
        childJnt = cmds.listRelatives( baseJnt, c=1, typ = "joint" )[0] 
        childJntName = cmds.rename( childJnt , name + "_ctlJnt"+str(i+1).zfill(2))
        
        jPos = cmds.xform( childJntName, q=1, ws=1, t=1  )
        
        #create ctl on the jnt
        jCtl = genericController( childJntName.replace("_ctlJnt", "ctl"), jPos, radius*0.5, "circle", 11 )    
        
        cmds.parent( jCtl[1],  baseCtl[0] )        
        cmds.parentConstraint( jCtl[0], childJntName, maintainOffset = 1 )
            
        baseJnt = childJntName
        baseCtl = jCtl
        ctls.append( jCtl[0] )
        
    cmds.select( baseCtl[0], r=1 )
    cmds.select( ctls, add=1 )
    cmds.controller( g =1 )
