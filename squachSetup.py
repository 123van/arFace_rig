...
def indieScaleSquach( squachNode, axis, name ):
    ...
    #pow operation
    if cmds.attributeQuery("squach", node = squachNode, exists=1)==False:
...
def mouthSquach_setup( head, ctl ):
    ...
    cmds.aimConstraint( bendLoc, bend[1], mo = 0, weight=1, aimVector = (1,0,0), upVector = (0,1,0), n = bend[0]+"aimConst" )
...
def squachSetup( jntLen ):
    ...
    if not geo:
        ...
        if not cmds.objExists("attachCtl_grp"):
            cmds.group(p="bodyHeadTRS", n="attachCtl_grp")
        squachCtlGrp = cmds.group( em=1, p = "attachCtl_grp", n ="squachCtl_grp"  ) 
        ...
        for ctlP in ctlGrpList:
            cmds.parent ( ctlP, squachCtlGrp )
        ...
        ikSpline = cmds.ikHandle( sj=stretchJntList[0], ee=stretchJntList[-1], sol = "ikSplineSolver", createCurve=1, n ="squach_ik", rootOnCurve=True, parentCurve=False, numSpans = 4 )
        ...
        cmds.addAttr( "topSquach_ctl", ln ="squach_jnts", dt = "stringArray"  )
        cmds.setAttr( "topSquach_ctl.squach_jnts", type="stringArray", *([len(skinJnts)] + skinJnts) )
...
def latticeSquachskinning(jntLen):
        
    geo = "latticeCube"
    if cmds.objExists(geo):
        skinJnts = cmds.getAttr( "topSquach_ctl.squach_jnts" )
        squachLattice = cmds.lattice( geo, divisions= [ 2, int(jntLen)+1, 5 ], objectCentered=1, ldivisions=[2,2,2], n = "squach_ffd1" )
        squachFFd = [ ffd for ffd in squachLattice if cmds.nodeType(ffd)=="ffd" ]
        latticeSkin = cmds.skinCluster( skinJnts, squachLattice[1], normalizeWeights= 1, n = "squachLattice_skin" )
        cmds.lattice( squachFFd[0], e=1, rm =1, geometry = geo )
        boxSkin = cmds.skinCluster( skinJnts, geo, normalizeWeights= 1, n = "squachBox_skin" ) 
