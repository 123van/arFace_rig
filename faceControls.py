...
def faceControlsUI():
    ...
    cmds.text( label = '')
    cmds.button( label = 'select ctl', bgc=[.82,.75,.10], command = select_mCtl )
    cmds.button( label = 'select ctl', bgc=[.82,.75,.10], command = select_ctl )
    cmds.text( label = '')
    cmds.button( label = 'reset ctl', bgc=[.82,.75,.10], command = reset_mCtl )
    cmds.button( label = 'reset ctl', bgc=[.82,.75,.10], command = reset_ctl )
    cmds.text( label = '')
    cmds.button( label = 'set keys', bgc=[.82,.75,.10], command = setKeysOnMCtl )
    cmds.button( label = 'set keys', bgc=[.82,.75,.10], command = setKeysOnCtl )
    cmds.text( label = '')
    cmds.button( label = 'delete CtrlKeys', bgc=[.82,.75,.10], command = deleteMCtlKeys )
    cmds.button( label = 'delete CtrlKeys', bgc=[.82,.75,.10], command = deleteCtrlKeys )
    spaceBetween(1,3)
    cmds.text( label = '')
    ...
    cmds.text( label = '/REGION ctl"!!       ',fn = "boldLabelFont")
    cmds.button( label = 'add items to exist-sets', bgc=[.82,.75,.10], command = addItems_set   )
    cmds.button( label = 'remove items from sets', bgc=[.82,.75,.10], command = removeItems_set   )
    spaceBetween(1,3)
    cmds.text( label = '')
...
def reset_ctl(*pArgs):
    ...
    if cmds.objExists("%s_set"%facePart ):
        twitchPanelConnect.resetCtrl(facePart)
    else:
        cmds.confirmDialog( title='Confirm', message= "%s_set"%facePart + " not exists" )
