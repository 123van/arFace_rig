...
def transfer_lipCrvBS( srcCrv, tgtCrv, wireName ):
    ...
    if lipCrv_pos[0] < 0.001:
        ...
        wtCrv = create_wtCircle( wireCrv, lipCrv_pos )        
...
def create_wtCircle( crv, pos):
    
    bbox = cmds.exactWorldBoundingBox( crv )
    ...
    if not len(mySel) >= 3:
        ...
        if  len(cmds.ls(headBase)) == 1:           
            ...
            if cmds.objExists(shapeCrv):
                ...
                attRotate = cmds.objExists( shapeCrv + '.' + attrN + "Wire_rot"  )
                attDropOff = cmds.objExists( shapeCrv + '.' + attrN + "DropOff"  )
...
def prototypeCrv_data( protCrvs ):
    ...
    for crv in protCrvs:
        ...
        dupCrv = cmds.rename( myCrv, crv.split("_")[0] + "tempCrv")
        ...
        if dupCrvShp:
            if cmds.nodeType(dupCrvShp[0]) == "nurbsCurve":
                ...
                upXyRatio = [  abs(topPos[1]-cornerPos[1])/cornerPos[0], abs(upFirstPos[1]-cornerPos[1])/upFirstPos[0], abs( upMidPos[2]-cornerPos[2])/(upMidPos[0]) ]                
                loXyRatio = [  abs(cornerPos[1]-bttmPos[1])/cornerPos[0], abs(cornerPos[1]-loFirstPos[1])/loFirstPos[0], abs(loMidPos[2]-cornerPos[2])/(upMidPos[0]) ]
